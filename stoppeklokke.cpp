...
int startStopButton{9}, resetButton{10}, recordedMinutes; // First and second variable is referencing the pin number of the button components used to start / stop and reset the display.
...
__attribute__((unused)) void loop() {
    ...
    if (digitalRead(startStopButton)) // Detect if the 'start / stop' button has been pressed.
    {
    ...
    while (startStopButtonHasBeenPressed) { // Makes the stop watch keep running until the stop button is pressed.
        if (startStopButtonHasBeenPressed) // Start or stop stopwatch when button is pressed.
        {
            ...
            if (digitalRead(startStopButton)) { // Detect if user is pressing button to stop.
