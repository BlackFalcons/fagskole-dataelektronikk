...
float elapsedSeconds = 0; // Declare variable with the floating point value data type.
...
__attribute__((unused)) void loop() {
    ...
    {
        elapsedSeconds = 0, recordedMinutes = 0; // Resetting the values storing the currently counted secondes and minutes.
        lcd.clear(); // Uses the clear method from the LiquidCrystal object removing any value currently written on the screen.
        lcd.setCursor(5, 0);
        lcd.print((String) recordedMinutes + ":" + (String) elapsedSeconds); // Updates LCD with: 0:0.0
    ...
    while (startStopButtonHasBeenPressed) { // Makes the stop watch keep running until the stop button is pressed.
        ...
        {
            if (elapsedSeconds >= 59.8) // Convert seconds to minutes, it is inaccurate by 20 ms beacuse the display is not responding fast enough.
            {
                tone(1, 2000); // Make a tone to warn the user that a minute has passed by.
                elapsedSeconds = 0; // Sets the second counter back to 0.
            ...
            delay(100);
            if (elapsedSeconds == 0) noTone(1);

            elapsedSeconds += 0.1; // Adding 100 ms to recorded time.
            lcd.clear();
            lcd.setCursor(5, 0);

            lcd.print((String) recordedMinutes + ":" + (String) elapsedSeconds); // Type casting recordedMinutes and elapsedSeconds to the type of string to print the values to the lcd screen.
